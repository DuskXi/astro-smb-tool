...
一个源:`packaging/icon.svg`。别的全从它渲 —— `uv run --extra qt python
scripts/make_icons.py`(**只在改了 SVG 之后跑**;产物提交进仓库,打包时不能
依赖一个要 QtSvg 的生成步骤,那会把 QtSvg 拖进发行包)。

出三样,给三个不同的消费方:

===========================================  ==========================
`packaging/icon.ico` / `.icns`                PyInstaller 的 `icon=`
`astro_smb_app/icons/app-{16..256}.png`       运行时 `setWindowIcon()`
同上                                          随 wheel 走(`pip install` 也有)
===========================================  ==========================

**exe 的图标和窗口的图标是两回事。** 只接前者的话,冻结之后任务栏有图标
而窗口标题栏是默认的 —— 而这要在那个地方看一眼才发现。`--selftest` 里
因此有一条查图标。

形是侧栏那个 `◉`:外环 + 实心核,三层同心圆。**判据是 16×16** —— 图标
九成的时间活在任务栏和 Finder 侧栏里。同一轮画过两个信息更多的候选
(叠传输箭头、装星野),16px 上一个糊成噪点、一个只剩个圆圈。
`tests/test_icon.py` 里那条量的就是这个:16px 那张里至少要有三种色块。
...
**Gatekeeper 只管带 `com.apple.quarantine` 这条扩展属性的文件**,而那条属性
是**下载它的那个程序**打上去的(浏览器、AirDrop、微信……),不是系统自动
加在所有文件上的。所以第一个分岔在这儿:
...
| 本机自己打的 | 没有 |
| `curl` / `wget` 下的 | 没有 |
| 浏览器下的压缩包,**终端里 `tar -xzf` 解开** | 归档自己有,**解出来的文件没有** |
| 浏览器下的压缩包,**访达里双击解开** | 有 —— 访达会把它传给解出来的每个文件 |

**没有隔离属性 = 一句话都不会弹。** 这不是绕过什么,是隔离属性本来就是
"下载器给可疑来源打的标",谁都没打就没有。
...
| 签名状态 | 用户看到 |
|---|---|
| Developer ID + **已公证** | 一句普通确认框:「这是从互联网下载的,确定打开吗?」→ **打开** |
| Developer ID 但**没公证** | 「无法验证开发者」,只有「移到废纸篓 / 取消」两个键 |
| ad-hoc / 自签名 / 没签 | 同上 |

两件事要特别说明,因为很容易想反:

- **"只要确认一下"那种体验,恰恰是公证过的才有。** 要跑去「系统设置 →
  隐私与安全性」点「仍要打开」的,是**验证更少**的那一档,不是更多。
  直觉是反的。
- **自签名证书买不到任何东西。** Gatekeeper 不认自签的根,表现和完全没签
  一模一样。它唯一的用处是给 App 一个稳定身份(钥匙串、防火墙提示不会每次
  重新问)。另外 Apple Silicon 上二进制**必须至少 ad-hoc 签**才能执行,
  而这一步 PyInstaller 已经替我们做了(构建日志里那句 `Re-signing the EXE`)。

所以在**没有用户**的阶段,99 美元/年买的其实是这一格:
"浏览器下载 + 访达解压"的那条路上,把「去设置里点仍要打开」换成
「点一下打开」。我们发的是 `.tar.gz`、目标用户又是会开终端的人,
**这一格现在不值 99 刀。**

要做的时候路线是清楚的:

1. Apple Developer Program,99 美元/年(个人也能办);
2. 签 **Developer ID Application** 证书(免费会员拿不到这一类);
3. 设 `ASTRO_SMB_CODESIGN_ID`,PyInstaller 会把收集到的**每一个**
...
5. `xcrun notarytool submit --wait` 交给苹果自动扫描(几分钟);
6. `xcrun stapler staple` 把票据钉进产物 —— 钉完首次启动才不用联网。

**第 6 步决定产物形态**:`stapler` 只认 `.app` / `.dmg` / `.pkg`,钉不了裸
可执行文件。也就是说"要不要做 `.app`"和"要不要公证"是同一个决定。
而 `.app` 又和现在的 `console=True` 冲突(缺依赖时那句提示得有地方显示)。

**发布包保持 `.tar.gz`,不要换成 `.dmg` 或 `.zip`** —— 换了就等于把上面那张
表里最省事的一行删掉。
